...
void loop()
{
  int pre_conn_button = 22;
  send_cmd(adjVol30);
  while (1)
  {
    digitalWrite(pin_tx, LOW);
    for ( buttonPin = 2; buttonPin <= 3; buttonPin++ )
    {
      buttonState = digitalRead(buttonPin);
      if (  buttonState == LOW ) //the button connected
      {
        connButton = buttonPin - 1;
        in_signal = 1;
      }
      else
      {
    ...
    Serial.println(connButton);
    if ( in_signal == 1 )
    {
      mySerial.setTX(pin_tx);
      set_track(setTrack0, connButton);
      pre_conn_button = connButton;
      Serial.println("changed");
      digitalWrite(pin_tx, LOW);
    }
  }
  //
  //if ( connButton != pre_conn_button)
